...
public:
    ...
    TraverseForScans(Vertex < T > *pStartVertex, Visitor < T > *pVisitor) = 0;
...
public:
      ...
      kt_double smearDeviation)
    {
      ...
      CorrelationGrid * pGrid = new CorrelationGrid(
        width, height, borderSize, resolution,
        smearDeviation);
     ...
     * @return grid index
     */
    virtual kt_int32s GridIndex(
      const Vector2 < kt_int32s > & rGrid,
      kt_bool boundaryCheck = true) const
    {
...
protected:
    ...
    virtual void CalculateKernel()
    {
      ...
      if (!math::InRange(
          m_SmearDeviation, MIN_SMEAR_DISTANCE_DEVIATION,
 ...
 */
  ...
  struct LocalizationScanVertex
  {
    ...
    LocalizationScanVertex(const LocalizationScanVertex & obj) {
      scan = obj.scan; vertex = obj.vertex;
    }
...
public:
    ...
    void AddScanToLocalizationBuffer(
      LocalizedRangeScan * pScan,
